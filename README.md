...
宏观事件长期历史会写入 `DATA_DIR/macro-history.sqlite3`，保存滚动三十天的事件明细和
真实来源检查记录；它与用于快速读取的 `macro-events-cache.json` 分开。若要跨 Docker
容器重建保留历史，必须把 `DATA_DIR` 挂载到 Mac 的本地文件夹。
...
| `MACRO_AUTO_REFRESH_INTERVAL_SEC` | `600`（10分钟） | 否；后台读取官方 RSS/事件源的间隔，最小60秒 |
...
状态摘要会为每个事件类型返回最近一次和下一次已缓存事件的官方标题、时间或日期及
时间精度；它不会返回官方链接、上游响应正文或任何密钥。
