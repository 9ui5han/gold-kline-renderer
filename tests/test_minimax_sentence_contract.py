...
class MiniMaxSentenceContractTests(unittest.TestCase):
    def test_parser_keeps_complete_segment_without_splitting_decimal_prices(self):
        ...
        self.assertEqual(units[0]["segment_id"], "technical_evidence_1")
        self.assertEqual(units[0]["parent_segment_id"], "technical_evidence")
    ...
    def test_parser_preserves_short_pause_exactly(self):
        units = main.parse_minimax_sentence_units(payload_for([{
            "order": 1,
            "segment_id": "closing",
        ...
        self.assertEqual([unit["pause_after_ms"] for unit in units], [40])
